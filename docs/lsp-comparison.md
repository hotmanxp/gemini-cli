...
**适用服务器**：typescript, vue, pyright, svelte, astro, yaml-ls, bash,
intelephense, gopls, rubocop, csharp-ls, fsautocomplete
...
const asset = release.assets.find(
  (item) => item.name.includes(token) && item.name.includes(tag),
);
