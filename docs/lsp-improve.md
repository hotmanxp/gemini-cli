...
**Gemini
CLI 仅支持 6 种语言**，而 OpenCode 支持 30+ 种，oh-my-opencode 支持 40+ 种。
...
2. **去抖动处理** (防止频繁通知)
   ...
   connection.onNotification('textDocument/publishDiagnostics', (params) => {
     ...
     this.diagnosticDebounceTimers.set(
       params.uri,
       setTimeout(() => {
...
通过实施这些优化，Gemini CLI
LSP 可以达到或超越 OpenCode 和 oh-my-opencode 的水平，同时保持自身的独特优势 (Warmup、重启机制、Call
Hierarchy、Code Actions)。
