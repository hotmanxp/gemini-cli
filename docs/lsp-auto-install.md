...
Gemini CLI 现在支持选择性自动下载 LSP 服务器。当检测到所需的 LSP 服务器未安装时，系统可以自动通过 npm/yarn/pnpm/pip 安装。
...
// 检查是否安装
const { installed, hint } = checkLspServerInstallation('typescript-language-server');
