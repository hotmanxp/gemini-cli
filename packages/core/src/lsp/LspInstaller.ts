...
export async function installLspServer(
  ...
  if (alreadyInstalled) {
    debugLogger.log(
      `LSP server ${serverName} (${checkCommand}) is already installed`,
    );
  ...
  debugLogger.log(
    `Install command: ${installCommand.join(' ')} ${packageName} ${installArgs.join(' ')}`,
  );
...
export function getInstallConfig(serverName: string): InstallOptions | null {
  ...
  const mappings: Record<
    string,
    { packageName: string; checkCommand: string }
  > = {
    typescript: {
