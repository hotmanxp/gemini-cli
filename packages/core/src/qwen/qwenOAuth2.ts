...
export async function getQwenOAuthClient(
  ...
  try {
    ...
    if (options?.requireCachedCredentials) {
      throw new Error(
        'Qwen OAuth credentials expired. Please use /qwen-auth to re-authenticate.',
      );
    }
  ...
  try {
    ...
    if (!config.isBrowserLaunchSuppressed() && config.isInteractive()) {
      try {
        ...
        debugLogger.debug('Failed to open browser:', err);
      }
    }
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      try {
        ...
        debugLogger.error('Polling error:', error);
        return {
          success: false,
          reason: 'error',
