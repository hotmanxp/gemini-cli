...
export async function readQwenCredentials(): Promise<QwenCredentials | null> {
  try {
    ...
    debugLogger.debug('Failed to read Qwen credentials:', error);
    return null;
  }
}
...
export async function clearQwenCredentials(): Promise<void> {
  try {
    ...
    debugLogger.debug('Cleared Qwen credentials from file');
  } catch (error) {
    // Ignore if file doesn't exist
    if ((error as NodeJS.ErrnoException).code !== 'ENOENT') {
      debugLogger.warn('Failed to clear Qwen credentials:', error);
    }
  }
}
 ...
 * - Race condition prevention through file locking
 */
export class SharedTokenManager {
  ...
  private async refreshCredentials(
    ...
    if (credentials?.access_token && !areCredentialsExpired(credentials)) {
      ...
      debugLogger.debug('Loaded valid credentials from file');
      return credentials;
    }
