...
export class OpenAIContentConverter {
  ...
  /**
   * Create a final response with accumulated reasoning content.
   * This should be called when the stream ends to emit any remaining
   * accumulated reasoning that wasn't emitted due to missing finish_reason.
   */
  createFinalReasoningResponse(): GenerateContentResponse | null {
    ...
    response.candidates = [
      {
        content: {
          parts: [{ text: this.accumulatedReasoning, thought: true }],
          role: 'model' as const,
        },
        index: 0,
        safetyRatings: [],
