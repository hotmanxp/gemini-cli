...
export interface OperationalGuidelinesOptions {
   ...
   * (e.g. ~/.gemini/AGENTS.md). Surfaced to the model when memoryV2Enabled
...
function toolUsageRememberingFacts(
  ...
  - **Global Personal Memory** (\`${options.globalMemoryPath}\`): Cross-project personal preferences and facts about the user that should follow them into every workspace (e.g. preferred testing framework across all projects, language preferences, coding-style defaults). Loaded automatically in every session. Keep entries concise and durable — never workspace-specific.`
    ...
    const globalRoutingRule = options.globalMemoryPath
      ? `
  - When the user states a **cross-project personal preference** that should follow them into every workspace ("I always prefer X", "across all my projects", "my personal coding style is Y", "in general I like Z"), update the global personal memory file. Do **not** also write it into a \`AGENTS.md\` file or the private memory folder.`
      : '';
    return `
- **Instruction and Memory Files:** You persist long-lived project context by editing markdown files directly with ${formatToolName(EDIT_TOOL_NAME)} or ${formatToolName(WRITE_FILE_TOOL_NAME)}. There is no \`save_memory\` tool. The current contents of all loaded \`AGENTS.md\` files and the private project \`MEMORY.md\` index are already in your context — do not re-read them before editing.
  ...
  - **Subdirectory Instructions** (e.g. \`./src/AGENTS.md\`): Scoped instructions for one part of the project. Reference them from \`./AGENTS.md\` so they remain discoverable.${userProjectBullet}${globalMemoryBullet}
  **Routing rules — pick exactly one tier per fact:**
  - When the user states a **team-shared convention, architecture rule, or repo-wide workflow** ("our project uses X", "the team always Y", "for this repo, always Z"), update the relevant \`AGENTS.md\` file. Do **not** also write it into the private memory folder or the global personal memory file.
  - When the user states a **personal-to-them local setup, machine-specific note, or private workflow** for this codebase ("on my machine", "my local setup", "do not commit this"), save it under the private project memory folder. Do **not** also write it into a \`AGENTS.md\` file or the global personal memory file.${globalRoutingRule}
