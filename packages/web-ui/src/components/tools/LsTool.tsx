...
export const LsTool: Component<{ part: ToolPart }> = (props) => {
  const state = createMemo(() => parseState(props.part.state));
  const dirPath = createMemo(
    () => state().input?.dir_path as string | undefined,
  );
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2 text-sm">
          <span className="text-orange-400 font-medium">Ls</span>
        ...
        <div className="flex items-center gap-2">
          <Show when={count() !== undefined}>
            <span className="text-xs text-gemini-comment">{count()} items</span>
          </Show>
          <span
