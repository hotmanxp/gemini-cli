...
async function initializeTheme(
  ...
  try {
    ...
    step.status = 'error';
    step.error =
      error instanceof Error ? error.message : 'Theme validation failed';
...
async function initializeSessionLogging(
  ...
  step: InitializationStepResult;
  result: { geminiMdFileCount: number };
}> {
  const step: InitializationStepResult = {
    id: 'session',
...
async function initializeIde(
  ...
  try {
    ...
    step.status = 'error';
    step.error =
      error instanceof Error ? error.message : 'IDE connection failed';
...
export async function initializeApp(
  ...
  const [
    { step: authStep, result: authResult },
    { step: themeStep, result: themeResult },
  ...
  const [{ step: sessionStep }, { step: ideStep }] =
    await backgroundInitPromise;
  ...
  return {
    ...
    geminiMdFileCount:
      sessionStep.status === 'completed' ? config.getGeminiMdFileCount() : 0,
  };
}
