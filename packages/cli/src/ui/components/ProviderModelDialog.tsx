...
import { ModelSlashCommandEvent, logModelSlashCommand, AuthType, debugLogger } from '@google/gemini-cli-core';
...
export function ProviderModelDialog({ onClose }: ProviderModelDialogProps): React.JSX.Element {
  ...
  const allModels = providerRegistry?.getAllModels() || new Map();
  
  const modelItems = useMemo(() => {
    const items = Array.from(allModels.entries()).map(([modelId, modelData]) => ({
      value: modelId,
  ...
  const initialIndex = useMemo(() => {
    ...
    const idx = modelItems.findIndex((item) => item.value === lastProviderModel);
    return idx >= 0 ? idx : 0;
  ...
  const handleSelect = useCallback(
    async (model: string) => {
      ...
      debugLogger.log('[ProviderModelDialog] config.getLastProviderModel() before:', config.getLastProviderModel());
      ...
      if (persistMode && loadedSettings) {
        debugLogger.log('[ProviderModelDialog] Calling saveModelChange to persist to settings.json');
        saveModelChange(loadedSettings, model);
      }
      
      debugLogger.log('[ProviderModelDialog] config.getLastProviderModel() after:', config.getLastProviderModel());
      debugLogger.log('[ProviderModelDialog] config.getModel() after:', config.getModel());
      ...
      if (newProvider !== currentProvider) {
        debugLogger.log('[ProviderModelDialog] Different provider, calling refreshAuth');
        await config.refreshAuth(AuthType.CONFIG_LOGIN);
      } else {
        debugLogger.log('[ProviderModelDialog] Same provider, calling switchModel');
        ...
        if (contentGenerator && 'switchModel' in contentGenerator) {
          ...
          debugLogger.log('[ProviderModelDialog] switchModel called with modelId:', modelId);
          await (contentGenerator as any).switchModel(modelId);
        } else {
          debugLogger.log('[ProviderModelDialog] switchModel not available, calling refreshAuth');
          // If switchModel not available, refresh auth to recreate content generator
          await config.refreshAuth(AuthType.CONFIG_LOGIN);
        }
      }
      debugLogger.log('[ProviderModelDialog] handleSelect completed, closing dialog');
      onClose();
    },
    ...
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold color={theme.status.error}>Error: Config not available</Text>
  ...
  if (modelItems.length === 0) {
    return (
      <Box
        ...
        width="100%"
      >
        <Text bold color={theme.status.error}>No Provider Models Available</Text>
