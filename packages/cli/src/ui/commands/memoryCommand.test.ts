  ...
  describe('/memory list', () => {
    ...
    beforeEach(() => {
      ...
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        ...
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} AGENTS.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No AGENTS.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
    ...
    it('should display a message if no AGENTS.md files are found', async () => {
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: 'No AGENTS.md files in use.',
        },
        expect.any(Number),
      );
    });
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/AGENTS.md', '/path/two/AGENTS.md'];
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 AGENTS.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
