...
export function UseAuthDialog({
  ...
  debugLogger.log(
    '[UseAuthDialog] initial showModelSelector:',
    showModelSelector,
  );
  ...
  const onSelect = useCallback(
    async (authType: AuthType | undefined, scope: LoadableSettingScope) => {
      debugLogger.log(
        '[UseAuthDialog.onSelect] called with authType:',
        authType,
      );
      if (exiting) {
        return;
      ...
      if (authType) {
        ...
        if (authType === AuthType.CONFIG_LOGIN) {
          ...
          debugLogger.log(
            '[UseAuthDialog.onSelect] CONFIG_LOGIN - opening model selector',
          );
          settings.setValue(scope, 'security.auth.selectedType', authType);
  ...
  const handleAuthSelect = (authMethod: AuthType) => {
    debugLogger.log(
      '[UseAuthDialog.handleAuthSelect] START - authMethod:',
      authMethod,
    );
    const error = validateAuthMethodWithSettings(authMethod, settings);
    debugLogger.log(
      '[UseAuthDialog.handleAuthSelect] validation error:',
      error,
    );
    if (error) {
      ...
      debugLogger.log(
        '[UseAuthDialog.handleAuthSelect] Calling onSelect with:',
        authMethod,
      );
  ...
  debugLogger.log(
    '[UseAuthDialog] showModelSelector:',
    showModelSelector,
    'rendering',
  );
  if (showModelSelector) {
