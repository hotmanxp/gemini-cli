...
function buildAvailableModels(
  ...
  const isQwenOAuth =
    selectedAuthType === AuthType.USE_QWEN ||
    process.env['USE_QWEN_OAUTH'] === 'true';

  // For Qwen OAuth, show only Qwen models
  if (isQwenOAuth) {
    const qwenOptions = [
      {
        value: QWEN_MODEL_AUTO,
        title: getDisplayString(QWEN_MODEL_AUTO),
        description:
          'Let Qwen CLI decide the best model for the task: coder-model, coder-model-lite',
      },
      {
        value: 'coder-model',
        title: getDisplayString('coder-model'),
        description:
          'Qwen Coder Model - Full capability model for complex tasks',
      },
      {
        value: 'coder-model-lite',
        title: getDisplayString('coder-model-lite'),
        description:
          'Qwen Coder Model Lite - Faster, lighter model for simple tasks',
      },
    ];
    ...
    return {
      availableModels: scaleOptions(qwenOptions),
      currentModelId: preferredModel || QWEN_MODEL_AUTO,
    };
  }
